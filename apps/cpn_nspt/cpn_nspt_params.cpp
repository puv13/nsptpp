///! \file
/// Getopt paramters for cpn_nspt.cpp - Option parser
...
static struct option long_options[] = {
  ...
  { "zeromodes", no_argument, &zeromodes, 1 },
  { "gaugefix", no_argument, &gaugefix, 1 },
...
void print_params()
{
  ...
  std::cout << std::setw(f1_width)
            << "\tGauge Fixing:" << (gaugefix == 0 ? "OFF" : "ON") << std::endl;
  std::cout << std::setw(f1_width)
            << "\tZero Mode sub.:" << (zeromodes == 0 ? "OFF" : "ON")
            << std::endl;
  std::cout << std::setw(f1_width)
